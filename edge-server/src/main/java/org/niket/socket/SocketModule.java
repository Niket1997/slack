...
public class SocketModule {
    ...
    public SocketModule(SocketIOServer socketIOServer, RedisMessageListenerContainer redisMessageListenerContainer, MessagingServiceClient messagingServiceClient, MessageEventListener messageListener) {
    ...
    private ConnectListener onConnected() {
        return client -> {
            ...
            Set<String> channels = userChannels.stream().map(it -> String.valueOf(it.getChannelId())).collect(Collectors.toSet());
            Set<ChannelTopic> channelTopics = userChannels.stream().map(it -> new ChannelTopic(String.valueOf(it.getChannelId()))).collect(Collectors.toSet());
