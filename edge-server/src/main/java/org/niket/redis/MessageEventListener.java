...
public class MessageEventListener implements MessageListener {
    ...
    public void onMessage(@NotNull Message message, byte[] pattern) {
        try {
            ...
            Optional<SocketIOClient> senderClient = socketIOServer.getAllClients().stream().filter(it -> it.get("userId") == messageEvent.getSenderUserId()).findFirst();
            if (senderClient.isEmpty()) {
                socketIOServer.getRoomOperations(String.valueOf(messageEvent.getChannelId())).sendEvent(readMessageEvent, messageEvent);
            } else {
                socketIOServer.getRoomOperations(String.valueOf(messageEvent.getChannelId())).sendEvent(readMessageEvent, senderClient.get(), messageEvent);
            }
        } catch (IOException e) {
